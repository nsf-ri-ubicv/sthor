...
def lcdnorm5(arr_in, neighborhood,
    ...
    if contrast:
        ...
        arr_sum = view_as_windows(arr_sum, (1, 1,
                                            1, nbw)).sum(-1)[:, :, :, ::stride, 
                                            0, 0, 0]
        arr_sum = view_as_windows(arr_sum, (1, 1,
                                            nbh, 1)).sum(-2)[:, :, ::stride, :,
    ...
    if divisive:
        ...
        arr_ssq = view_as_windows(arr_ssq, (1, 1,
                                            1, nbw)).sum(-1)[:, :, :, ::stride,
                                            0, 0, 0]
        arr_ssq = view_as_windows(arr_ssq, (1, 1,
                                            nbh, 1)).sum(-2)[:, :, ::stride, :,
